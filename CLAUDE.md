...
Every module keeps an `INSIGHTS.md` — what earlier sessions learned the hard
way. Read the relevant one **before** starting non-trivial work, and add to it
afterwards only when something genuinely new turned up. The
`engineering-insights` skill holds the rubric, the quality bar, and the
deduplication procedure — use it for both halves.
