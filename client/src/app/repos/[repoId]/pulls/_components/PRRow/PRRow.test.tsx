/**
 * The FINDINGS column has two jobs the row must not get wrong: chips reflect the
 * server's per-severity counts (zero counts stay hidden, an unreviewed PR shows
 * nothing at all), and the hover preview only costs a request once the user
 * actually hovers — hence the mocked `usePrReviews` that returns data only for a
 * non-null PR id.
 */
...
describe("PRRow findings column", () => {
  it("renders one chip per severity with findings, hiding zero counts", () => {
    renderRow(PR);
    expect(screen.getByTitle("Show only CRITICAL findings").textContent).toContain("2");
    expect(screen.getByTitle("Show only WARNING findings").textContent).toContain("1");
