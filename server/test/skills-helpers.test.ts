...
import { toSkillPromptBlock, withSkillStats } from '../src/modules/reviews/helpers.js';
...
describe('withSkillStats', () => {
  const assembly = { system: 's', skills: '### a\nA.\n\n### b\nB.', user: 'u' };
  const count = (text: string) => text.length;

  it('weighs the skills block alone and each skill in prompt order', () => {
    const out = withSkillStats(
      assembly,
      [
        { name: 'a', block: '### a\nA.' },
        { name: 'b', block: '### b\nB.' },
    ...
    expect(out.skills_tokens).toBe(assembly.skills.length);
    expect(out.skill_blocks).toEqual([
      { name: 'a', tokens: 8 },
      { name: 'b', tokens: 8 },
    ]);
  });

  it('returns the assembly untouched when no skill was injected', () => {
    const bare = { system: 's', skills: null, user: 'u' };
    expect(withSkillStats(bare, [], count)).toBe(bare);
  });
});
