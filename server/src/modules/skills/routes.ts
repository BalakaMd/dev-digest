...
import { MAX_BODY_BYTES, MAX_NAME_CHARS } from './constants.js';

/**
 * Skills module. A skill is reusable review guidance: text + config, never
 * code. Skills are workspace-scoped and shared across agents; the agent side of
 * the link (which skills, in what order) lives in the agents module.
 *
 *   GET    /skills                     → list with agent counts
